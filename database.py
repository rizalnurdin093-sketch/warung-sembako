...
def transaksi_add(nama_produk, harga_satuan, qty, kasir=None):
    total = harga_satuan * qty
    conn = get_db()
    conn.execute("INSERT INTO transaksi (nama_produk, harga_satuan, qty, total, kasir) VALUES (?,?,?,?,?)",
                 (nama_produk, harga_satuan, qty, total, kasir))
    conn.commit()
    conn.close()
...
def rekap_per_kasir():
    """Total penjualan per kasir (untuk lihat siapa menyumbang berapa)."""
    conn = get_db()
    rows = conn.execute("""
